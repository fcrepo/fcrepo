...
public class FedoraResponseCodesIT extends AbstractResourceIT {
    ...
    public void testAllowedAddDatastream() throws Exception {
        // 2Permit is appended because the tests are checking for it
        final String pid = getRandomUniquePid() + "2Permit";
    ...
    public void testDeniedAddDatastream() throws Exception {
        ...
        final HttpPost obj2Method = postObjMethod(pid + "/FedoraDatastreamsTest2Deny");
    ...
    public void testAllowedAddDeepDatastream() throws Exception {
        final String pid = getRandomUniquePid() + "_permit";
    ...
    public void testDeniedAddDeepDatastream() throws Exception {
        ...
        final HttpPost method =
                postDSMethod(pid + "/does_permit/not_permit/exist_permit/yet_permit/allowed_child", "zxc", "foo");
        final HttpResponse response = client.execute(method);
        assertEquals(403, response.getStatusLine().getStatusCode());
    }
    ...
    public void testDeniedPutDatastream() throws Exception {
        // Include _permit because we're testing against it
        final String pid = getRandomUniquePid() + "_permit";

        final HttpPost objMethod = postObjMethod(pid + "/allowed_child");
        assertEquals(201, getStatus(objMethod));
