...
public class FedoraLdp extends ContentExposingResource {
    ...
    public Response createOrReplaceObjectRdf(
        ...
        try {
            ...
            if (nodeService.exists(session.getFedoraTransaction(), path)) {
                resource = resource();
    ...
    public Response updateSparql(final InputStream requestBodyStream)
        ...
        final AcquiredLock lock = lockManager.lockForWrite(resource().getPath(), session.getFedoraTransaction(),
                nodeService);
    ...
    public Response createObject(@HeaderParam(CONTENT_DISPOSITION) final ContentDisposition contentDisposition,
        ...
        final AcquiredLock lock = lockManager.lockForWrite(newObjectPath, session.getFedoraTransaction(), nodeService);
    ...
    private void checkExistingAncestor(final String path) {
        ...
        while (!(parentPath.isEmpty() || parentPath.equals("/"))) {
            if (nodeService.exists(session.getFedoraTransaction(), parentPath)) {
    ...
    private FedoraResource createFedoraResource(final String path, final String interactionModel,
        ...
        if ("ldp:NonRDFSource".equals(interactionModel) || contentExternal ||
            ...
            result = binaryService.findOrCreate(session.getFedoraTransaction(), path);
            timeMapService.findOrCreate(session.getFedoraTransaction(), path + "/" + FEDORA_DESCRIPTION);
        } else {
            result = containerService.findOrCreate(session.getFedoraTransaction(), path, interactionModel);
        }

        timeMapService.findOrCreate(session.getFedoraTransaction(), path);
    ...
    private String mintNewPid(final String slug) {
        ...
        if (nodeService.exists(session.getFedoraTransaction(), pid)) {
