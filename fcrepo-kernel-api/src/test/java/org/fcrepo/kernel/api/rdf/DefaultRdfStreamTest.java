...
public class DefaultRdfStreamTest {
    ...
    public final void testMap() {
        ...
        final RdfStream stream = new DefaultRdfStream(subject, getTriples(subject).stream());

        final List<String> objs = stream.map(Triple::getObject)
            .map(Node::getURI).collect(toList());
    ...
    private List<Triple> getTriples(final Node subject) {
