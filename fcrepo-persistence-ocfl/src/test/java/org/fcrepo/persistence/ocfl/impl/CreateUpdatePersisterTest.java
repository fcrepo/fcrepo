...
public class CreateUpdatePersisterTest {
    ...
    private final CreateUpdatePersister persister = new CreateUpdatePersister();
    ...
    @Test
    public void testRdf() throws PersistentStorageException {
        ...
        when(rdfSourceOperation.getTriples()).thenReturn(userTriplesStream);
        when(rdfSourceOperation.getServerManagedProperties()).thenReturn(serverTriplesStream);
        persister.persist(session, rdfSourceOperation, mapping);
    ...
    public void testNonRdf() throws Exception {
        ...
        when(nonRdfSourceOperation.getFilename()).thenReturn("myHappyFile.txt");
        ...
        // verify user content
        verify(session).write(eq("child.txt"), userTriplesIsCaptor.capture());
        final InputStream userContent = userTriplesIsCaptor.getValue();
        assertEquals(inputContent, IOUtils.toString(userContent, "UTF-8"));
    ...
    @Test
    public void testNonRdfNoFilename() throws Exception {
        ...
        final RdfStream serverTriplesStream = new DefaultRdfStream(resourceUri, serverTriples);
        when(nonRdfSourceOperation.getResourceId()).thenReturn(resourceId);
        when(nonRdfSourceOperation.getFilename()).thenReturn(null);
        when(nonRdfSourceOperation.getContentStream()).thenReturn(content);
        when(nonRdfSourceOperation.getServerManagedProperties()).thenReturn(serverTriplesStream);
        ...
        // verify user content
        verify(session).write(eq("child"), userTriplesIsCaptor.capture());
        final InputStream userContent = userTriplesIsCaptor.getValue();
        assertEquals(inputContent, IOUtils.toString(userContent, "UTF-8"));
